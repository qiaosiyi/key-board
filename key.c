...
//·¢ËÍÃüÁîºó,¼ì²â½ÓÊÕµ½µÄÓ¦¹q
//str:ÆÚ´ýµÄÓ¦´ð½á–¨
//·µ»Ø‚ž:0,Ã»ÓÐµÃµ½ÆÚ´ýµÄÓ¦´ð½á–¨
//    ÆäËû,ÆÚ´ýÓ¦´ð½á¹ûµÄÎ»ÁZ(strµÄÎ»ÁZ)
u8* check_cmd(u8 *str)
{
	char *strx=0;
	if(USART_RX_STA&0X8000)		//½ÓÊÕµ½Ò»´ÎÊý¾ÝÁË
	{ 
		USART_RX_BUF[USART_RX_STA&0X7FFF]=0;//Ìí¼Ó½áÊø¹J
...
//Ïòsim900a·¢ËÍÃü·Â
//cmd:·¢ËÍµÄÃüÁî×Ö·ûØ¯(²»ÐèÒªÌí¼Ó»Ø³µÁË),µ±cmd<0XFFµÄÊ±‚ž,·¢ËÍÊýŒ\(±ÈÈç·¢ßT0X1A),´óÓÚµÄÊ±ºò·¢ËÍ×Ö·û´®.
//ack:ÆÚ´ýµÄÓ¦´ð½á–¨,Èç¹ûÎª¿Õ,Ôò±íÊ¾²»ÐèÒªµÈ´ýÓ¦¹q
//waittime:µÈ´ýÊ±¼ä(µ¥Î»:10ms)
//·µ»Ø‚ž:0,·¢ËÍ³ÉÊÆ(µÃµ½ÁËÆÚ´ýµÄÓ¦´ð½á¹û)
//       1,·¢ËÍÊ§»ß
u8 send_cmd(u8 *cmd,u8 *ack,u16 waittime)
{
	...
	if((u32)cmd<=0XFF)
	{
		while((USART1->SR&0X40)==0);//µÈ´ýÉÏÒ»´ÎÊý¾Ý·¢ËÍÍê·¿  
		USART1->DR=(u32)cmd;
	}else printf("%s\r\n",cmd);//·¢ËÍÃü·Â
	if(ack&&waittime)		//ÐèÒªµÈ´ýÓ¦¹q
	{
